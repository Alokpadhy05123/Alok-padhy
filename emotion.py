...
def generate_priors(
    ...
    print("Total Approx Priors Numbers:{}".format(len(priors)))
    print('=================================')
...
def camera():
    import emoji_dictonary
    cap = cv2.VideoCapture('ved4.mp4')
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            for (x1, y1, x2, y2) in boxes:
                ...
                pred = emoji_dictonary.emotion_dict[list(output[0]).index(max(output[0]))]
                print(pred)
                cv2.rectangle(
                    img_ori, 
                    (x1, y1), 
